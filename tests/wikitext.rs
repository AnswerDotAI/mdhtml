...
fn templates_are_semantic_instructions() {
    let html = wiki2mdhtml("Distance: {{convert|12|km|abbr=on}}; {{{label|unknown}}}; {{#if:x|yes|no}}.").unwrap();
...
fn media_is_a_structured_image() {
    let html = wiki2mdhtml("Before [[File:Plot.png|thumb|Plot]] after").unwrap();
