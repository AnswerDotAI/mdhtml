...
impl SourceLocation { pub const fn new(line: usize, column: usize) -> Self { Self { line, column } } }
...
impl SourceSpan {
    pub const fn bytes(start: usize, end: usize) -> Self { Self { start, end, start_location: None, end_location: None } }
...
pub struct Line<'a> { raw: &'a str }

#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub struct LineOffset { pub byte: usize, pub column: usize, pub blank: bool }

impl<'a> Line<'a> {
    pub const fn new(raw: &'a str) -> Self { Self { raw } }
    pub fn indent(self) -> usize { self.first_nonspace().column }
    pub fn first_nonspace(self) -> LineOffset { self.first_nonspace_from(0, 0) }
    ...
    pub fn byte_at_column(self, target: usize) -> Option<usize> {
        ...
        for (idx, ch) in self.raw.char_indices() {
            if col == target { return Some(idx); }
            match ch { ' ' => col += 1, '\t' => col = next_tab(col), _ => return (col == target).then_some(idx) }
            if col > target { return None; }
        }
        (col == target).then_some(self.raw.len())
    }

    pub fn strip_indent(self, columns: usize) -> String { self.strip_from(0, 0, columns) }

    pub fn strip_from(self, start_byte: usize, start_column: usize, target_column: usize) -> String {
        let mut col = start_column;
        let raw = &self.raw[start_byte..];
        for (idx, ch) in raw.char_indices() {
            if col >= target_column { return expand_leading_indent(&raw[idx..], col); }
            match ch {
                ' ' => col += 1,
                '\t' => {
...
fn next_tab(col: usize) -> usize { col + TAB_STOP - (col % TAB_STOP) }
